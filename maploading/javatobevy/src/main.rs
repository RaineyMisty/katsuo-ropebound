...
fn load_map_data(
    ...
    // read Json document
    let json_str = std::fs::read_to_string("assets/allFunctionality.json")
    ...
    std::fs::write("./assets/temp_tile_fg.png", &tile_fg_bytes)
        .expect("Failed to write temp tile image");
    std::fs::write("./assets/temp_entity.png", &entity_bytes)
